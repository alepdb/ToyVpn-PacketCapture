...
static char *get_file_name() 
{
    ...
    if(strftime(buffer, 50, "%Y-%m-%d_%H-%M-%S", ptr_time) == 0) {
        perror("Couldn't prepare formatted string");
        return NULL;
    } else {
        file_name = strcat(buffer, ".pcap");
        return file_name;
    }

}

static const char *FILE_NAME = get_file_name();

static void save_packet(u_char *packet, int packet_len)
{
    //Create dumper to write on file
    pcap_t *p = pcap_open_dead(DLT_RAW, 65535); //DLT_RAW = DLT_IPV4 & DLT_IPV6
    pcap_dumper_t *dumper = pcap_dump_open_append(p, FILE_NAME);

    //Create the header and insert packet length
    struct pcap_pkthdr header;
    header.caplen = packet_len;
    header.len = packet_len;

    //Calculate timestamp with microseconds precision and insert into packet header
...
//     pcap_t *handle = pcap_open_live("lo", BUFSIZ, 0, 1000, errbuf);
...
//     pcap_dumper_t *dumper = pcap_dump_open_append(handle, FILE_NAME);
...
int main(int argc, char **argv)
{
    if(FILE_NAME){
        ...
        perror("Could not calculate file name\n");
        return(3);
    }
    ...
    while ((tunnel = get_tunnel(argv[2], argv[3])) != -1)
    {
        ...
        while (true)
        {
            ...
            //Receiving from interface (network)
            if (length > 0)
            {
                printf("Recv: %d\n", length);
                
                //Save packet in .pcap file
                save_packet(packet, length);
            ...
            //sniffPacket = pcap_next(tunHandle, &header);
            //pcap_dump((u_char *)dumper, &header, sniffPacket);
        }
        printf("%s: The tunnel is broken\n", argv[1]);
        //Closing Pcap

        // pcap_dump_close(dumper);
        // pcap_close(tunHandle);
